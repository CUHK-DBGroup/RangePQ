...
int PQKMeans::predict_one_single(const std::vector<unsigned char> &pyvector)
{
    assert(pyvector.size() == M_);
    std::pair<std::size_t, float> nearest_one = FindNearetCenterLinear_single(pyvector, centers_);
    return (int) nearest_one.first;
}
...
std::pair<std::size_t, float> PQKMeans::FindNearetCenterLinear_single(const std::vector<unsigned char> &query,
                                                               const std::vector<std::vector<unsigned char> > &codes)
{
    std::vector<float> dists(codes.size());
