...
public:
    ...
    int predict_one_single(const std::vector<unsigned char> &pyvector);
...
private:
    ...
    std::pair<std::size_t, float> FindNearetCenterLinear_single(const std::vector<unsigned char> &query,
                                                         const std::vector<std::vector<unsigned char>> &codes);
