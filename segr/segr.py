...
class Segr(object):
    ...
    def __init__(self, fine_quantizer):
        ...
        self.impl_cpp = main.SegrCpp(fine_quantizer.codewords, fine_quantizer.verbose)
    ...
    def reconfigure(self, nlist=None, iter=5, cz=20000):
    ...
    def add(self, vecs, keySet, valueSet, update_posting_lists="auto"):
        ...
        assert keySet.ndim == 1
        assert valueSet.ndim == 1

        self.impl_cpp.add_codes(self.fine_quantizer.encode(vecs), keySet, valueSet, 
                                self._resolve_update_posting_lists_flag(update_posting_lists))

    def add_encoded(self, vecs, keySet, valueSet, update_posting_lists="auto"):
        """Push back new vectors to the system.
        More specifically, this function
        (1) encodes the new vectors into PQ/OPQ-codes,
        (2) pushes back them to the existing database PQ/OPQ-codes, and
        ...
        self.impl_cpp.add_codes(vecs, keySet, valueSet, 
                                self._resolve_update_posting_lists_flag(update_posting_lists))

    def erase(self, key):
        self.impl_cpp.delete_key(key)

    def add_configure(self, vecs, keySet, valueSet,nlist=None, iter=5, cz = 20000):
        ...
        print("addBegin")
        self.add(vecs=vecs, keySet = keySet, valueSet = valueSet,update_posting_lists=False)
        self.reconfigure(nlist=nlist, iter=iter,cz=cz)
        return self
    
    def add_configure_encoded(self, vecs, keySet, valueSet,nlist=None, iter=5, cz = 20000):
        """Run :func:`add` (with ``update_postig_lists=False``) and :func:`reconfigure`.
        To add vectors for the first time, please call this.
        ...
        print("addBegin")
        self.add_encoded(vecs=vecs, keySet = keySet, valueSet = valueSet,update_posting_lists=False)
        self.reconfigure(nlist=nlist, iter=iter,cz=cz)
        return self

    def add_configure_encoded_2(self, vecs, keySet, cor,ass, valueSet,nlist=None, iter=5, cz = 20000):
        """Run :func:`add` (with ``update_postig_lists=False``) and :func:`reconfigure`.
        To add vectors for the first time, please call this.
        ...
        print("addBegin")
        self.add_encoded(vecs=vecs, keySet = keySet, valueSet = valueSet,update_posting_lists=False)
        self.reconfigure_2(nlist=nlist, iter=iter,cz=cz,cor = cor,ass = ass)
        return self
    
    def reconfigure_2(self, cor, ass, nlist=None, iter=5, cz=20000):
        """Given a new ``nlist``, update the :attr:`coarse_centers` and
        :attr:`posting_lists` by grouping the stored PQ-codes (:attr:`codes`)
        into ``nlist`` clusters.
        ...
        self.impl_cpp.reconfigure_2(nlist, iter,cz, cor, ass)
    ...
    def query(self, q, topk=1, L=None, range_l=None, range_r=None, method = "auto"):
        ...
        if method == "segr":
            ids, dists = self.impl_cpp.query_seg_range(q_, topk, range_l, range_r, L)
            # print(ids)
